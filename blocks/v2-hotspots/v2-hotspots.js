...
function addMobileAlternativeCards(hotspot, main) {
  ...
  hotspot.text.forEach(p => {
    ...
    pEl.innerHTML = p.innerHTML;
    featureBlock.querySelector('.feature').appendChild(pEl);
  });
...
function updateDesktopLayoverBeforeAndNextButtons(block) {
  ...
  dialogs.forEach((dialog, index) => {
    ...
    dialog.querySelector('span.hotspot-layover-pagination').innerHTML = `${prevIndex + 1}/${nextIndex + 1}`;  
  });
}
...
function addMobileTabNavigation(block, title, hotspotAreaId) {
  ...
  li.addEventListener('click', (event) => {
    ...
    document.querySelectorAll('.section.v2-hotspots-container.active')
      .forEach((activeHotspotArea) => activeHotspotArea.classList.remove('active'));

    // activate the new hotspot area
    const targetHotspotArea = document.querySelector(`.section.v2-hotspots-container[data-hotspot-area-id="${id}"]`);
